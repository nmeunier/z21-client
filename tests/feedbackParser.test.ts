...
describe("FeedbackParser", () => {
  ...
  it("should parse LAN_RMBUS_DATACHANGED (groupIndex 0) as an occupancy snapshot", () => {
    const payload = Buffer.from([
      0x0f, 0x00, 0x80, 0x00,
      0x00, 0x01, 0x00, 0xc5, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ]);
    const result = new FeedbackParser().parse(payload);
    expect(result?.type).toBe("occupancy");
    const value = (result as any).value;
    expect(value.bus).toBe("rbus");
    expect(value.channels).toHaveLength(80);
    expect(value.channels.filter((c: any) => c.occupied)).toEqual([
  ...
  it("should parse LAN_RMBUS_DATACHANGED (groupIndex 1) with +10 address offset", () => {
    const payload = Buffer.from([
      0x0f, 0x00, 0x80, 0x00,
      0x01, 0x01, 0x00, 0xc5, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ]);
    const result = new FeedbackParser().parse(payload);
    expect((result as any).value.channels.filter((c: any) => c.occupied)).toEqual([
