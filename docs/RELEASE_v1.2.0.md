...
- Added `engines.cvReadIndexed(indexHigh, indexLow, cv)` and `engines.cvWriteIndexed(indexHigh, indexLow, cv, value)` to `EngineController`, implementing NMRA indexed CV access (CV31/CV32 page registers, S-9.2.2 Appendix B). This lets callers read/write CVs beyond the direct-mode range (257-512 window) by first writing the index registers CV31/CV32, then reading/writing the target CV. Closes [#3](https://github.com/nmeunier/z21-client/issues/3).
